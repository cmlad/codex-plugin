...
Whenever you push code in this workflow, you MUST immediately leave a top-level PR comment summarizing what feedback, review threads, or failing checks that push addressed. This is in addition to any thread-specific replies used to resolve comments.

## Workflow

1. Identify the PR for the current branch with `gh pr view --json number,url,headRefName`.
2. Push any pending fixes. If you push code, immediately leave the required PR comment summarizing what feedback or failures that push addressed.
...
4. If actionable unresolved feedback remains, fix it directly, push, leave the required PR comment summarizing what feedback that push addressed, and repeat Step 3.
...
8. Fix PR-caused failures directly, push, leave the required PR comment summarizing what failures or feedback that push addressed, and go back to Step 3.
